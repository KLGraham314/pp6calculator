...
double quadraticplus(double a, double b, double d){ //Function to find the larger real solution to a quadratic equation
	return (((-1)*b)+ sqrt((b*b)-(4*a*d)))/(2*a);
}
double quadraticminus(double a, double b, double d){ //Function to find the smaller real solution to a quadratic equation
	return (((-1)*b)- sqrt((b*b)-(4*a*d)))/(2*a);
}
...
double flength(double a, double b, double d, double e){ //Function to find the length of a 4-vector
	...
	if(sumsqr>=0) return sqrt(sumsqr);
	if(sumsqr<0) return sqrt(-1*sumsqr);
}
double invmass(double E1, double E2, double px1, double px2, double py1, double py2, double pz1, double pz2){ //Function to calculate invariant mass of two particles
	double sumsqr = ((E1+E2)*(E1+E2)) - ((px1+px2)*(px1+px2)) - ((py1+py2)*(py1+py2)) - ((pz1+pz2)*(pz1+pz2));
	if(sumsqr>=0) return sqrt(sumsqr);
	if(sumsqr<0) return -1; // E^2<p^2 is unphysical
}
	...
	while(j==0){ //While j remains zero, the calculator will keep performing functions until user quits
		...
		if(!std::cin){ //If input failed, move onto the next iteration
			...
			if(inside>=0){ //If inside is positive the answer will be real
				c=quadraticplus(a,b,inside);
				cminus=quadraticminus(a,b,inside);
			...
			c = invmass(E1,E2,px1,px2,py1,py2,pz1,pz2);
			if(c<0){ //If E^2<p^2, c will be -1
